...
function Profile(props) {
  ...
  const drawer = (
    <div>
      ...
      <Divider />
      <List>
        {[<Button variant="text" href="/transfer"> Transfer</Button>, 'Messages', 'Send email', 'Drafts'].map((text, index) => (
      ...
      <Divider />
      <List>
        {['All mail', 'Trash', <span onClick={() => signOut(auth)}>Sign Out</span>].map((text, index) => (
          <ListItem key={text} disablePadding>
            <ListItemButton>
              <ListItemIcon>
                {index % 2 === 0 ? <InboxIcon /> : <MailIcon />}
